...
if (-not $gitCmd) {
    Write-Host "  -> Git not found. Installing Git for Windows..." -ForegroundColor White
...
Write-Host "  [PASS] Git is ready." -ForegroundColor Green

# 2. Check / Install Python 3.11
Write-Host "`n[2/6] Checking Python installation..." -ForegroundColor Yellow
$pyCmd = Get-Command python -ErrorAction SilentlyContinue
if (-not $pyCmd) {
    Write-Host "  -> Python not found. Installing Python 3.11 (64-bit)..." -ForegroundColor White
...
Write-Host "  [PASS] Python is ready." -ForegroundColor Green
...
Write-Host "  [PASS] Repository is cloned at: $targetDir" -ForegroundColor Green
...
Write-Host "1. Edit .env with your MT5 credentials:" -ForegroundColor White
Write-Host "   code $targetDir\.env" -ForegroundColor Gray
Write-Host "   (Enter MT5_LOGIN, MT5_PASSWORD, MT5_SERVER, and SYMBOLS=XAUUSD.x,EURUSD.x)`n" -ForegroundColor Gray
